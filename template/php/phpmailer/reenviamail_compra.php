<?
				...
				$cuerpo.= "<p><b>Direcci&oacute;n: </b>".htmlentities($row2['direccion'])."</p>";
				$cuerpo.= "<p><b>Pa&iacute;s: </b>".htmlentities($row2['pais'])."</p>";
				$cuerpo.= "<p><b>Provincia: </b>".htmlentities($row2['provincia'])."</p>";
				$cuerpo.= "<p><b>Poblaci&oacute;n: </b>".htmlentities($row2['poblacion'])."</p>";
				...
				while ($row=recibir_array($result)){
					...
					$cuerpo.= "<td style='border: 1px solid #ccc;'>".utf8_decode($row[3]). "</td>";
				...
				if(enviaEmilio2("info@mo-el.es,$row2[email]","Factura mo-el.es",$cuerpo,"Factura mo-el.es",$quien,"info@mo-el.es")){

					echo "<p>Mail enviado correctamente a: <b>".$row2['email']."</b> con copia a: info@mo-el.es";
					?><script type="text/javascript"> alert("Mail enviado correctamente.") </script><?
					?><script>document.location.href="../../../themes/admin/ventas_view.php?id=<?=$id_comanda?>";</script><?php
				}
				else echo "Mail no enviado";
